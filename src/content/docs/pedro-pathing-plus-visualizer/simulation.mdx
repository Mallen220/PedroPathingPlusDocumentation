...
description: "Visualize FTC robot movement, physics, and timing with the Pedro Pathing Plus Visualizer. Ensure your paths are safe and efficient before running them on your robot."
---

# Simulation & Playback

Visualize your robot's path with a physics-based simulation in the FTC path planner. This tool helps you verify timing, check for collisions, and ensure smooth motion before running code on the real robot.
...
Enable these overlays in **[Settings](/pedro-pathing-plus-visualizer/settings/)** to better understand robot behavior:
...
- **Collision Detection**: Potential collisions with field boundaries or defined obstacles are marked with warning indicators. [Learn more about Path Validation](/pedro-pathing-plus-visualizer/validation/).
- **Real-time Feedback**: The "Current Robot Position" panel displays live X/Y coordinates and heading during playback.
- **Event Markers**: Visual dots on the timeline show exactly when events (like "Intake" or "Shoot") will trigger. [Learn more about Event Markers](/pedro-pathing-plus-visualizer/event-markers/).
