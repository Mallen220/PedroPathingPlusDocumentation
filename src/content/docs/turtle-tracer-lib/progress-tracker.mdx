...
head:
  - tag: meta
    attrs:
      name: keywords
      content: TurtleTracerLib, ProgressTracker, robot progress monitoring, FTC path events, event triggering, robotics
---

The `ProgressTracker` is a powerful utility class in TurtleTracerLib that allows you to track the percentage of completion of a path, path chain, or even a turning operation. Its primary purpose is to simplify the triggering of events (like moving an arm or spinning an intake) based on how far the robot has traveled along its current trajectory.
