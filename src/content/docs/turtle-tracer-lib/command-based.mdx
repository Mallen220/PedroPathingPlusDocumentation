...
description: Learn how to use FTC Command-Based programming with TurtleTracerLib for efficient and structured robot code using CommandScheduler, Subsystems, and FollowPathCommand.
head:
  - tag: meta
    attrs:
      name: keywords
      content: TurtleTracerLib, command-based programming, FTC, robotics, WPILib, FTCLib, CommandScheduler, FollowPathCommand, Subsystem, SequentialCommandGroup
---

TurtleTracerLib includes built-in support for Command-Based programming, allowing you to structure your robot code efficiently using commands and subsystems. This approach is similar to [WPILib](https://docs.wpilib.org/) and [FTCLib](https://docs.ftclib.org/) but tailored specifically for the FTC TurtleTracerLib library.
...
The `com.turtletracerlib.command` package provides several key classes:
...
TurtleTracerLib provides several standard commands to simplify common tasks:
...
import com.turtletracerlib.command.Subsystem;
import com.turtletracerlib.command.Command;
import com.turtletracerlib.command.RunCommand;
...
### Using ParallelRaceGroup for Timeouts
...
Since TurtleTracerLib does not provide a base `CommandOpMode`, you must manage the `CommandScheduler` yourself in a `LinearOpMode`.
...
import com.turtletracerlib.command.SequentialCommandGroup;
import com.turtletracerlib.command.ParallelCommandGroup;
import com.turtletracerlib.command.WaitCommand;
