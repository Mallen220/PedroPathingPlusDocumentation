...
head:
  - tag: meta
    attrs:
      name: keywords
      content: TurtleTracer, custom plugins, exporters, FTC tools, themes, robotics
---

# Plugins

TurtleTracer supports a powerful plugin system that allows you to extend its functionality. You can add custom code exporters, UI themes, and even new interface elements using simple JavaScript files.
...
turtle.registries.navbarActions.register({
  ...
  title: "Say Hello",
  onClick: () => {
    alert("Hello from TurtleTracer Plugin!");
  },
});
```
